...
const itemUtil = require('./util/item-util');
...
function handleAddItemIntent(intent, session, response) {
    ...
    const result = itemUtil.getItemFromIntent(intent);
    result.on('found', function found(item) {
    ...
    result.on('fuzzyFound', function fuzzyFound(fuzzyItem) {
    ...
    result.on('notFound', function nothingFound(item) {
...
function handleAddSomeItemIntent(intent, session, response) {
    ...
    result.on('found', function found(item) {
        ...
        session.attributes.quantity = null;
...
function handleQuantityNumberIntent(intent, session, response) {
    ...
    if (Number.isInteger(item)) {
        client.addItemToBasket(itemId, quantity, function callback(error) {
            const message = messageBuilder.buildAddItemResponseMessage(error, item, item.quantity);
            response.tell(message);
        });
    } else {
...
function handleYesIntent(intent, session, response) {
    ...
    const rootIntent = session.attribute.rootIntent;
