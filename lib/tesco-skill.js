...
function handleQuantityNumberIntent(intent, session, response) {
    ...
    const item = {
        name: session.attributes.itemName,
    ...
    if (!isNaN(quantity)) {
        client.addItemToBasket(item, quantity, function callback(error) {
            console.info(`Added to the basket ${item.name} with id ${item.id}`);
...
function handleNoIntent(intent, session, response) {
    ...
    console.info(`Item ${itemName} is not correct, should I add it anyway?`);
