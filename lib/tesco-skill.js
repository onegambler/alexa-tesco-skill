 ...
 */
function handleAddItemIntent(session, response) {
    console.info(`ADDING stuff by calling ${process.env.TRIGGER_URL}`);
    client.addItem('277925449', function callback(error) {
        let message;
        if (error) {
            message = 'Something went wrong';
        } else {
            message = 'I added what you asked to the shopping list';
        }
...
TescoSkill.prototype.intentHandlers = {
    AddItem(intent, session, response) {
        handleAddItemIntent(session, response);
    },
