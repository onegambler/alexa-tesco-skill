...
function handleAddItemIntent(intent, session, response) {
    ...
    console.info(`Id ${itemIdentifier} for ${item}`);
    if (item && !itemIdentifier) {
        console.info('Running fuzzy search');
        const fuzzyItem = fuzzy.findBestMatch(item);
        console.info(`best match ${fuzzyItem}`);
        if (fuzzyItem) {
            ...
            response.ask(`Sorry I didn't quite get that. Do you mean ${fuzzyItem}`);
        } else {
